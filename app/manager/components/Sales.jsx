...
export default function Sales({ employees }) {
  const [from, setFrom] = useState(startOfWeek())
  const [to, setTo] = useState(todayStr())
  const [sales, setSales] = useState([])
  const [itemsBySale, setItemsBySale] = useState({})
  ...
  async function toggleExpand(sale) {
    ...
    if (!itemsBySale[sale.id]) {
      const { data } = await supabase
        ...
        .eq('sale_id', sale.id)
        .order('product_name')
      setItemsBySale(prev => ({ ...prev, [sale.id]: data || [] }))
    }
  }

  const totalRevenue = sales.reduce((sum, s) => sum + Number(s.total), 0)
  const itemCount = (sale) => itemsBySale[sale.id]?.reduce((sum, i) => sum + i.quantity, 0)
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <div>
          <div style={{ color: '#bfe3cf', fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Sales</div>
          <div style={{ color: 'white', fontSize: 26, fontWeight: 900 }}>{sales.length}</div>
        </div>
      </div>
      ...
      <div style={{ background: 'white', borderRadius: 12, border: '1px solid #e5e7eb', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
          ...
          <tbody>
            {loading && <tr><td colSpan={4} style={{ padding: 20, textAlign: 'center', color: '#9ca3af' }}>Loading…</td></tr>}
            {!loading && sales.length === 0 && (
              <tr><td colSpan={4} style={{ padding: 20, textAlign: 'center', color: '#9ca3af' }}>No sales in this range.</td></tr>
            )}
            {sales.map(sale => (
              <Fragment key={sale.id}>
                <tr style={{ borderTop: '1px solid #f0f0f0', cursor: 'pointer' }} onClick={() => toggleExpand(sale)}>
                  <td style={{ padding: '10px 14px' }}>{fmtWhen(sale.created_at)}</td>
                  <td style={{ padding: '10px 14px', fontWeight: 600 }}>{nameById[sale.employee_id] || 'Unknown'}</td>
                  <td style={{ padding: '10px 14px', fontWeight: 700, color: GREEN }}>$${Number(sale.total).toFixed(2)}</td>
